...
public class CalculatorTest {
    ...
    public void inverseGivesCorrectResultForPositiveValues() {
        ...
        for (int i = 1; i < 10; i++) {
            expected = 1.0/i;
            calculator.setValue(expected);
            assertEquals(expected, calculator.getValue(), ALLOWED_ERROR);
        }
    }
    ...
    @Test (expected=IllegalArgumentException.class)
    public void inverseGivesIllegalArgumentExceptionWhenDividingByZero() {
        calculator.inverse();
    }

    @Test 
    public void inverseTwiceGivesOriginalValue() {
        for (int i = 1; i < 10; i++) {
            double expected = i;
            calculator.setValue(i);
            calculator.setValue(calculator.inverse());
            calculator.setValue(calculator.inverse());
            assertEquals(expected, calculator.getValue(), ALLOWED_ERROR);
        }
    }
